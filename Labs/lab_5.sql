...
SELECT * FROM Person WHERE Income > 450 AND Age > 37 OR Hours < 40 AND Income > 250;

-- 4) Create table users and add constraint for email that checks for existing @
CREATE TABLE Users (
    first_name VARCHAR(255),
    last_name VARCHAR(255),
    email VARCHAR(255) CONSTRAINT checker CHECK (email LIKE '%@%')
);
...
SELECT * FROM Users WHERE first_name LIKE 'D%r%';
-- 7) Delete from table person those 
-- who never married and age over 30
DELETE FROM Person WHERE Marital = 'never' AND Age > 30;
...
substring(email FROM '\.[a-z]+@[a-z]+\.') AS "Without com and name" FROM Users;
...
DELETE FROM Users WHERE email LIKE '%yahoo%' OR last_name LIKE '%s%' OR last_name LIKE '%S%';
