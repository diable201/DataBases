...
SELECT warehouse, count(*) FROM Packs GROUP BY warehouse HAVING count(*) > 2;
-- 9. Create a new warehouse in Texas
-- with a capacity for 5 packs.
INSERT INTO Warehouses (location, capacity) VALUES ('Texas', 5);
...
UPDATE Packs SET value = value * 0.82 WHERE code = (
SELECT code FROM Packs ORDER BY value DESC LIMIT 1 OFFSET 2
);
...
DELETE FROM Packs WHERE warehouse IN (
